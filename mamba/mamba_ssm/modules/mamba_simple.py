...
from mamba_ssm.ops.selective_scan_interface import selective_scan_fn, mamba_inner_fn, selective_scan_forward
...
class Mamba(nn.Module):
    ...
    def forward(self, hidden_states, inference_params=None):
        ...
        if inference_params is not None:
            if inference_params.draft_model:
                conv_state, ssm_state = self._get_states_from_cache(inference_params, batch)
                if inference_params.seqlen_offset > 0:
                    # The states are updated inplace
                    out, _, _ = self.step(hidden_states, conv_state, ssm_state)
                    # save state into state cache for backtracking
                    # inference_params.prev_memory_list.append({self.layer_idx: (next_conv_state.clone(), next_ssm_state.clone())})
                    return out
            else:
                if inference_params.is_backtrack:
    ...
    def resume_forward(self, hidden_states, conv_state, ssm_state):
        """
        hidden_states: (B, L, D)
        Returns: same shape as hidden_states
        """
    ...
    def _get_states_from_cache_verifier(self, inference_params, batch_size):
        ...
        if self.layer_idx not in inference_params.key_value_memory_dict:
            ...
            # for memorize old state
            prev_conv_state = torch.zeros(
                batch_size,
                self.d_conv - 1,
                self.d_model * self.expand,
                device=self.conv1d.weight.device,
